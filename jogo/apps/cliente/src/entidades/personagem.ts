...
export function atualizarPersonagem(p: Personagem, controles: Controles, dt: number, tempo: number): void {
  ...
  if (pular && !p.pularSegurado && p.noChao) {
  ...
  atualizarAnjo(p.anjo, dt, tempo, p, poseDe(p));
}
...
// Em coordenadas do mapa: as asas atrás, o sprite e, na frente, o que o anjo põe por cima.
