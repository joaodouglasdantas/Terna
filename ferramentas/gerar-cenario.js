// Gera os sprites do cenário a partir das folhas de referência em assets/fontes/.
// Uso: node ferramentas/gerar-cenario.js
// Saída: assets/cenario.png (tudo empacotado) e src/cenario-quadros.js (recortes).

const fs = require('fs');
const path = require('path');
const { lerPng, escreverPng } = require('./png');

const RAIZ = path.join(__dirname, '..');
const FONTES = path.join(RAIZ, 'assets', 'fontes');
const SAIDA_PNG = path.join(RAIZ, 'assets', 'cenario.png');
const SAIDA_JS = path.join(RAIZ, 'src', 'cenario-quadros.js');

const TELA = { altura: 270 }; // mesma altura do canvas do jogo
const ESCALA_ARVORES = 1 / 3;
...
function recortarGrupo(arquivo, { alfaMin, vizinhanca, tamanhoMin, cortesY, escala, vento = false }) {
...
fs.writeFileSync(
  SAIDA_JS,
  '// Gerado por ferramentas/gerar-cenario.js a partir de assets/fontes/ — não editar à mão.\n' +
    ...
    `const QUADROS_CENARIO = ${JSON.stringify(quadros)};\n`,
);
console.log(
